...
<form id="form_salva" method="post">
    ...
    <input type="text" name="DataFinal" placeholder="Digite seu nome" required>
    <input type="text" name="DataInicio" placeholder="Digite seu sobrenome" required>
    <input type="text" name="Obs" placeholder="Digite seu telefone" required>
    <input type="text" name="OrdemServico" placeholder="Digite seu email" required>
    <input type="text" name="StatusServico" placeholder="Digite seu banco" required>
    <input type="text" name="TipoServico" placeholder="Digite seu cep" required>
...
if(
    ...
    isset($_POST['StatusServico']) &&
    isset($_POST['TipoServico'])
){

$CPF= limpaPost($_POST['CPF']);
$DataFinal= limpaPost($_POST['DataFinal']);
$DataInicio= limpaPost($_POST['DataInicio']);
$Obs= limpaPost($_POST['Obs']);
$OrdemServico= limpaPost($_POST['OrdemServico']);
$StatusServico= limpaPost($_POST['StatusServico']);
$TipoServico= limpaPost($_POST['TipoServico']);
...
if ($StatusServico == "" || $StatusServico == null){
    echo "<b style='color:red'>Insira um banco</b>";
    exit();
}

if ($TipoServico == "" || $TipoServico == null){
    echo "<b style='color:red'>Insira um cep</b>";
    exit();
}


$sql = $pdo->prepare("INSERT INTO tblservico VALUES (?, ?, ?, ?, ?, ?, ?)");
$sql->execute(array($CPF, $DataFinal, $DataInicio, $Obs, $OrdemServico, $StatusServico, $TipoServico));
...
$sql = $pdo->prepare("SELECT CPF, DataFinal, DataInicio, Obs, OrdemServico, StatusServico, TipoServico FROM tblservico");
$sql->execute();
$dados = $sql->fetchAll();
...
if(count($dados) > 0){
    ...
    foreach($dados as $chave => $valor){
        echo " <tr>
            <td>".$valor['CPF']."</td>
            <td>".$valor['DataFinal']."</td>
            <td>".$valor['DataInicio']."</td>
            <td>".$valor['Obs']."</td>
            <td>".$valor['OrdemServico']."</td>
            <td>".$valor['StatusServico']."</td>
            <td>".$valor['TipoServico']."</td>
